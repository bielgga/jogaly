...
export default function JogosMultiplayerPage() {
  const [games, setGames] = useState<GameListItem[]>([])
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)

  useEffect(() => {
    async function loadMultiplayerGames() {
      setLoading(true)
      setError(null)
      
      try {
        // Temporariamente usando jogos da página 2 para multiplayer
        // Você pode criar uma página específica para multiplayer depois
        const gamesData = await gameService.getGamesByPage(2)
        const sortedGames = gamesData.sort((a, b) => b.likes - a.likes)
        setGames(sortedGames)
        
        if (gamesData.length === 0) {
          setError('Nenhum jogo multiplayer encontrado.')
        }
      } catch (err) {
        console.error('Erro ao carregar jogos multiplayer:', err)
        setError('Erro ao carregar jogos multiplayer. Tente novamente mais tarde.')
      }
    ...
    <div>
      ...
      <div className="mb-12">
        ...
        <p className="text-center text-gray-300 mt-6 text-lg">
          Desafie amigos e jogadores do mundo todo! Jogue os melhores jogos multiplayer online grátis com partidas em tempo real, cooperação e competição épica.
        </p>
      </div>
