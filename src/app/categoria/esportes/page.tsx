...
export default function JogosEsportesPage() {
  ...
  useEffect(() => {
    async function loadSportsGames() {
      ...
      try {
        // Carregar jogos da página 8 (jogos de esportes)
        const gamesData = await gameService.getGamesByPage(8)
        const sortedGames = gamesData.sort((a, b) => b.likes - a.likes)
        setGames(sortedGames)
        
        if (gamesData.length === 0) {
          setError('Nenhum jogo de esportes encontrado.')
        }
      } catch (err) {
        console.error('Erro ao carregar jogos de esportes:', err)
        setError('Erro ao carregar jogos de esportes. Tente novamente mais tarde.')
      }
    ...
    <div>
      ...
      <div className="mb-12">
        ...
        <p className="text-center text-gray-300 mt-6 text-lg">
          Viva a emoção do esporte! Futebol, basquete, tênis e muito mais. Jogos esportivos gratuitos com simulação realista e jogabilidade viciante.
        </p>
      </div>
