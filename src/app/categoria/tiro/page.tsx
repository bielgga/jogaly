...
export default function JogosTiroPage() {
  const [games, setGames] = useState<GameListItem[]>([])
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)

  useEffect(() => {
    async function loadShootingGames() {
      setLoading(true)
      setError(null)
      
      try {
        // Carregar jogos da página 5 (jogos de tiro)
        const gamesData = await gameService.getGamesByPage(5)
        const sortedGames = gamesData.sort((a, b) => b.likes - a.likes)
        setGames(sortedGames)
        
        if (gamesData.length === 0) {
          setError('Nenhum jogo de tiro encontrado.')
        }
      } catch (err) {
        console.error('Erro ao carregar jogos de tiro:', err)
        setError('Erro ao carregar jogos de tiro. Tente novamente mais tarde.')
      }
    ...
    <div>
      ...
      <div className="mb-12">
        ...
        <p className="text-center text-gray-300 mt-6 text-lg">
          Mire, atire e acerte o alvo! Jogos de tiro com ação intensa, precisão milimétrica e desafios que testam seus reflexos e estratégia.
        </p>
      </div>
