...
export default function JogosCozinharPage() {
  const [games, setGames] = useState<GameListItem[]>([])
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)

  useEffect(() => {
    async function loadCookingGames() {
      setLoading(true)
      setError(null)
      
      try {
        // Carregar jogos da página 4 (jogos de cozinhar)
        const gamesData = await gameService.getGamesByPage(4)
        const sortedGames = gamesData.sort((a, b) => b.likes - a.likes)
        setGames(sortedGames)
        
        if (gamesData.length === 0) {
          setError('Nenhum jogo de cozinhar encontrado.')
        }
      } catch (err) {
        console.error('Erro ao carregar jogos de cozinhar:', err)
        setError('Erro ao carregar jogos de cozinhar. Tente novamente mais tarde.')
      }
    ...
    <div>
      ...
      <div className="mb-12">
        ...
        <p className="text-center text-gray-300 mt-6 text-lg">
          Torne-se um chef master! Cozinhe pratos deliciosos, gerencie restaurantes e aprenda receitas nos melhores jogos de culinária online gratuitos.
        </p>
      </div>
