...
export default function JogosCorridaPage() {
  const [games, setGames] = useState<GameListItem[]>([])
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)

  useEffect(() => {
    async function loadRacingGames() {
      setLoading(true)
      setError(null)
      
      try {
        // Carregar jogos da página 6 (jogos de corrida)
        const gamesData = await gameService.getGamesByPage(6)
        const sortedGames = gamesData.sort((a, b) => b.likes - a.likes)
        setGames(sortedGames)
        
        if (gamesData.length === 0) {
          setError('Nenhum jogo de corrida encontrado.')
        }
      } catch (err) {
        console.error('Erro ao carregar jogos de corrida:', err)
        setError('Erro ao carregar jogos de corrida. Tente novamente mais tarde.')
      }
    ...
    <div>
      ...
      <div className="mb-12">
        ...
        <p className="text-center text-gray-300 mt-6 text-lg">
          Acelere a fundo e sinta a adrenalina! Carros esportivos, motos velozes e pistas desafiadoras nos melhores jogos de corrida online gratuitos.
        </p>
      </div>
